...
from utils.evaluate import mae, rmse

# %%
magpie_features, labels = load_gaultois(target_cols=["icsd_id", "formula", "T", "zT"])

labels, magpie_features = dropna(labels, magpie_features)


# %%
[X_train_mp, y_train_mp], [X_test_mp, y_test_mp] = train_test_split(
    magpie_features, labels
)
...
labels.to_csv(ROOT + "/data/for_roost.csv", index=False, float_format="%g")
...
roost_featurizer = Roost(
    ...
    n_targets=1,  # number of labels to fit roost to, just 1 usually
