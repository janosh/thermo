...
for label, y_test, y_pred, y_std in zip(
    labels.columns, y_norm.values.T, rf_y_pred.values.T, rf_y_var.values.T ** 0.5
):
...
# %%
for label, y_test, y_pred, y_std in zip(
    labels.columns, y_norm.values.T, do_y_pred.values.T, do_y_var.values.T ** 0.5
):
    plot_output(y_test, y_pred, y_std, title=label)
...
# %%
for label, y_test, y_pred, y_std in zip(
    labels.columns, y_norm.values.T, gp_y_pred.values.T, gp_y_var.values.T ** 0.5
):
    plot_output(y_test, y_pred, y_std, title=label)
